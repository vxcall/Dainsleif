...
void HookLockCursor() {
    ...
    uintptr_t dwCinput = PatternScanner("client.dll", "\xB9????\xF3\x0F\x11\x04\x24\xFF\x50\x10", 1).CalculateOffset(Modules::client, 0);
    std::cout << Modules::client + dwCinput << std::endl;
    input = **reinterpret_cast<CInput***>(Modules::client + dwCinput);
